...
zandile({
    ...
    try {
        // Set the correct timezone, e.g., Africa/Gweru. You can change it to match your region
        const currentTime = moment().tz("Africa/Gweru").format("HH:mm:ss");
        const currentDate = moment().tz("Africa/Gweru").format("dddd, MMMM Do YYYY");
        ...
        const sendMenuImage = async () => {
            try {
                return await conn.sendMessage(
                    from,
                    {
                        image: { url: config.MENU_IMAGE_URL || 'https://files.catbox.moe/dnjqau.jpg' },
                        caption: menuCaption,
                        contextInfo
                    },
                    { quoted: mek }
